...
def experiment(num_step, T=1, population_size=512, scaling=0.1, noise=1, weight_decay=0):
    ...
    x = torch.randn(population_size, 83)
    ...
    random_map = RandomProjection(83, 2, normalize=True)

    for t, alpha in tqdm(scheduler, total=scheduler.num_step-1):
        rewards, obs = compute_rewards_list(6, 3, 8, x * scaling)
...
def make_video(para):
    ...
    model = ControllerMLP.from_parameter(6, 3, 8, para)
    controller = DiscreteController(model, env.action_space)
